...
function LogInForm() {
  ...
  const isFormValid = jmbg && password;
  ...
  const handleSubmit = (event) => {
    ...
    if(!isFormValid){
      setShowValidation(true);
        ...
        axios.post(`${data.ssoServiceUrl + "Login"}`,dto).then(
          (res) => {
  ...
  return (
      <>
        {show &&(
          ...
          <Alert.Heading>Neispravni korisnički podaci!</Alert.Heading>
        </Alert>
     )}
      {showValidation &&(
          <Alert variant="danger" onClose={() => setShowValidation(false)} dismissible>
          <Alert.Heading>Molim vas popunite sva polja!</Alert.Heading>
        </Alert>
     )}
      ...
      <Card.Body className='text-center d-flex flex-column  justify-content-center align-items-center'>
        ...
        <Form style={{ width: '80%' }} onSubmit={handleSubmit}>
          <Form.Group className="mb-3" controlId="formBasicEmail">
            ...
            <Form.Control type="text" value={jmbg} style={{ fontSize: '1.2em' ,padding: '10px'}}onChange={handleJmbgChange} placeholder="Unesite jmbg" />
          </Form.Group>
          <Form.Group className="mb-5" controlId="formBasicPassword">
            <Form.Label style={{ fontSize: '1.8em' }} className='text-light'>Lozinka:</Form.Label>
            <Form.Control style={{ fontSize: '1.2em' ,padding: '10px'}} value={password} onChange={handlePasswordChange} type="password" placeholder="Lozinka" />
          </Form.Group>
